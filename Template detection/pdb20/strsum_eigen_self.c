...
#define MAXSEQLEN 3000				//最长序列长度
#define MAXWINLEN 250				//最长win长度？
#define MAXATOMS 30000

#define SQR(x) ((x)*(x))			//平方操作
...
/* Constants for calculating CB coords (Prot. Eng. Vol.2 p.121) 用于计算CB坐标的常量*/
...
/* DSSP residue solvent accessible area in GGXGG extended pentapeptide		 GGXGG延伸五肽中DSSP残留溶剂可达区域*/
...
/* Structure generation arrays				结构生成数组*/
float           (**distmat)[4][4];
...
/* Dump a rude message to standard error and exit 将粗鲁的消息转储到标准错误并退出*/
void
                fail(char *errstr)
{
    fprintf(stderr, "\n*** %s\n\n", errstr);
...
float           torsion(const float *a, const float *b, const float *c, const float *d)
{
    ...
    /* Calculate vectors and lengths a-b, b-c, c-d				计算向量和长度a-b, b-c, c-d*/
    for (i = 0; i < 3; i++)
    {
	a_b[i] = b[i] - a[i];
	b_c[i] = c[i] - b[i];
    ...
    /* Error check, are any vectors of zero length ?			错误检查，是否任意长度为零的向量*/
    if ((len_a_b == 0.0) || (len_b_c == 0.0) || (len_c_d == 0))
	return (-999.0);

    /* Calculate dot products to form cosines					 计算点积来形成余弦*/
    ...
    /* If the angle is not == 180 degs calculate sign using sine	如果角度不是== 180度，使用正弦计算符号 */
    if (costsq < 1.0)
    {
	sintor = sqrt(1.0 - costsq);
	tor = atan2(sintor, costor);			//返回以弧度表示的 y/x 的反正切
	tor /= conv;							//conv = 0.01745329

	/* Find unit vectors							找到单位向量 */
	for (i = 0; i < 3; i++)
	{
	    a_b[i] /= len_a_b;
	    b_c[i] /= len_b_c;
...
/* Summarize PDB/DSSP file */			//总结PDB / DSSP文件
int
                summary(FILE * pfp, FILE * dfp, FILE * ofp)
{
	...
	if (strstr(buf, "NUMBER OF CHAINS"))					//strstr作用是返回字符串中首次出现子串的地址。
	{
	    if (sscanf(buf, "%d%d%d%d%d", &nres, &i, &i, &i, &i) != 5)		//读取格式化的字符串中的数据
	...
	if (seq[i] != GAP)
	{
		...
		{
		    /* Not much left of this residue!					剩下的残基不多了*/
		    printf("WARNING: Missing main chain atom in %d!\n", i + 1);
		    seq[i] = GAP;
		    continue;
		}
		
		/* Reconstruct CB atom									重建CB原子*/
...
/* Eigen decomposition code for symmetric matrix, taken/modified from the public
   domain Java Matrix library JAMA. 对称矩阵的本征分解代码，取自/修改自公共领域Java矩阵库JAMA。*/

#define hypot2(x, y) (sqrtf((x)*(x)+(y)*(y)))

/* Symmetric Householder reduction to tridiagonal form. 对称的Householder简化为对角线形式*/
...
void tred2(float **V, float *d, float *e, int NDIM)
{
    ...
    /* Householder reduction to tridiagonal form.			Householder简化为对角线形式*/

    for (i = NDIM-1; i > 0; i--)
    {
	/* Scale to avoid under/overflow. 缩放以避免溢出/下溢*/

	float scale = 0.0F;
	float h = 0.0F;
	
	for (k = 0; k < i; k++)
	    scale = scale + fabsf(d[k]);					//absf处理float类型的取绝对值
	...
	{
	    /* Generate Householder vector. 生成Householder矢量		U酉矩阵*/
	    ...
	    /* Apply similarity transformation to remaining columns.将相似性转换应用于剩余的列 */
    ...
    /* Accumulate transformations. 累积转换*/
...
void tql2(float **V, float *d, float *e, int NDIM) 
{
    ...
    for (l = 0; l < NDIM; l++)
    {
	/* Find small subdiagonal element */						//找到小的次对角线元素
	...
	if (m > l)
	{
		...
		/* Implicit QL transformation. */				//隐式QL转换
		...
		for (i = m-1; i >= l; i--)
		{
		    ...
		    /* Accumulate transformation. */				//积累转变。
		...
		/* Check for convergence. */					//检查收敛性
		
	    } while (fabsf(e[l]) > eps*tst1);
	}
	
	d[l] = d[l] + f;
	//printf("d[l]==========%f\n", d[l]);// -1.132226	0.089612	12.287438
	e[l] = 0.0F;
    }
	//printf("d[2]==========%f\n", d[l]);
	/*
	...
	*/
    /* Sort eigenvalues and corresponding vectors. */	//对特征值和相应的向量进行排序
	int a = 0;		//记录大于0的数量
	float dsum;
	//printf("NDIM==========%d\n", NDIM);
	for (i = 0; i < NDIM - 1; i++)//	146
	{
		...
		if (k != i)		//交换两者
		{
			...
			for (j = 0; j < NDIM; j++)
			{
				...
				V[j][i] = V[j][k];
				V[j][k] = p;
				//printf("v[i3]==========%f\n",  V[j][k]);//v[i3]==========0.062106 v[i3]==========0.051752 v[i3]==========-0.030507
	...
	*/
	...
	for (i = 0; i < NDIM - 1; i++) 
	{
		...
		//printf("===================================================="); 
		B[i] =(d[i] - d[NDIM - 2]) * 1.0F / (d[0] - d[NDIM - 2]);
		//printf("%f\n", B[i]);
		Bsum += B[i];
	...
	*/
	...
	//xxy修改
	/*
	FILE* fpWrite = fopen("/share/home/zhanglab/xxy/Threader/Engerthread/database/AFDB_20W/num_0.7.txt", "a+");
	fprintf(fpWrite, "%d\n", R);
	printf("R=======%d\n", R);		//选择特征值个数
	for (i = R; i < NDIM-1; i++) {
		d[i] = 0;
...
/* Return eigenvectors (in V) and eigenvalues (in D) for symmetric matrix A 返回对称矩阵A的特征向量（V）和特征值（D）*/
...
/* Convert AA letter to numeric code (0-22) 将AA字母转换为数字代码(0-22) */
int
                aanum(int ch)
{
    static int      aacvs[] =
...
/* Convert string to lower case 将字符串转换为小写*/
void
                lc_str(char *str)
{
    int             i;
...
int             main(int argc, char **argv)
{
    ...
    ofp = fopen(argv[3], "w");			//tdb //w打开只写文件，若文件存在则文件长度清为0，即该文件内容会消失。若文件不存在则建立该文件。
    ...
    ofp = fopen(argv[4], "w");			//eig	//wb只写打开或新建一个二进制文件；只允许写数据。
	...
	//printf("eigenval===== %5d %s\n", (float*)eigenval);		//eigenval===== 12322256  *(float*)eigenval===== 8226832  (float*)eigenval===== 32916944
	//printf("eigenvec[0]====== %5d %s\n", & &eigenvec[0]);		//eigenvec[0]====== 12236976  
	...
	//printf("eigenval===== %5d %s\n", *(float*)eigenval);
	//printf("eigenvec[0]====== %5d %s\n", (float*)eigenvec[0]);	//eigenvec[0]====== 26876080  
	
    if (fwrite(eigenval, sizeof(float), nres, ofp) != nres)				//若写入元素总数不等于nres，则报错。
