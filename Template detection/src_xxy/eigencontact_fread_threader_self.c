...
char            brkid[10], path[500], seq1[MAXSEQLEN], seq2[10000];
...
void tql2(float **V, float *d, float *e, int NDIM)
{
	...
	//xxy修改
	FILE* fpWrite = fopen("/share/home/zhanglab/xxy/meta_contact/test_291/test_self_0.5_140/num.txt", "a+");
	fprintf(fpWrite, "%d\n", R);
	printf("a====%d\n", R);
	for (i = R; i < NDIM; i++) {
		d[i] = 0;
...
//            {
 ...
 ////               for (sum=k=0; k < neig; k++)
 //                    sum += eigenvec4[i][k] * eigenvec3[j][k] * eigenval4[k] * eigenval3[k] * sign[k];
				/*printf("%d %d %f\n", i, j, sum);*/
//                pmat[j][i] += sum * SCALE;
 //           }
...
maketemplate(FILE *dfp, int distflg)
{
	...
	resid[i] = strdup(ri);			//resid中存储的是TDB文件中的后半段

	cp = buf + 181;
	for (j = 0; j < 20; j++,cp+=6)
	{
	  consv = atoi(cp);
	  tpltsc1[i][j] = consv;       //将TDB文件中后半段的数据放入tpltsc1
	}
	...
	{
	    seq1[i] = aanum(buf[5]);	//将seq1的序列存放到seq1[i]数组
	    tsstruc[i] = buf[7];		//将TDB文件中的第8列字符存放到tsstruc[i]数组
	    sscanf(buf + 9, "%d", &j);
	    trelacc[i] = j;				//将TDB文件中第十一列的数字一次读入trelacc数组
	    whichatm[i] = 31;
	}
	++i;
    }
...
int             getpsipredv(FILE * lfil)
{
    ...
    naa = 0;
    while (!feof(lfil))			//feop检查文件是否读取结束
    {
	if (!fgets(buf, 256, lfil))
	    break;
...
/*int MIone(FILE *sfp)
{
	...
	solmap = allocmat(seq1len, seq1len, sizeof(float), TRUE);

	while(fgets(buf0, 256, sfp)){
		if (sscanf(buf0, "%d%d%f", &a, &b, &e) != 3)
			break;
		solmap[a-1][b-1] = solmap[b-1][a-1] = e;
		c++;
		printf("r===%f\n",e );
	}
...
int MItwo(FILE* stfp, float** mtx)
{
	...
	for (;;)
	{
		if (!fgets(buf, 256, stfp))
			break;


		if (sscanf(buf, "%d%d%f", &i, &j, &r) != 3)
			break;

		mtx[i - 1][j - 1] = mtx[j - 1][i - 1] = r;
		n++;
...
void sort(struct HIT* a, int l)//a为数组地址，l为数组长度。
{
	int i, j;
	struct HIT v;
	//排序主体
	for (i = 0; i < l - 1; i++)
		for (j = i + 1; j < l; j++)
		{
			if (a[i].contactsc < a[j].contactsc)//如前面的比后面的大，则交换。
			{
				v = a[i];
				a[i] = a[j];
				a[j] = v;
...
void sort1(struct HIT* a, int cc, int nn)//a为数组地址，l为数组长度。
{
	int i, j;
	struct HIT v;
	//排序主体
	for (i = cc; i < nn - cc - 1; i++)
		for (j = i + 1; j < nn - cc; j++)
		{
			if (a[i].contactsc < a[j].contactsc)//如前面的比后面的大，则交换。
			{
...
{
    int             aa, i, j, k, l, nn = 0, mm = 0, bb = 0, cc = 0, filtered, start1, start2, end1, end2, alnmode1, alnmode2, nid;
    char           *cmdstr, *cp;
    ...
    ifp = fopen(argv[1], "r");			//ifp目标的contact文件
    if (!ifp)
...
//		inputmat[i][j] = inputmat[j][i] = contmap[i][j] > 0.05;
	...
	// printf("argv[4]=====%s\n",argv[4]);
	 ...
	 //stfp=fopen(argv[4],"r");
	// if (!stfp)
	 //fail("Unable to open MItwo file!");
	 //soltwomap = allocmat(seq2len, seq2len, sizeof(float), TRUE);
	 //MItwo(stfp,soltwomap);
	 //fclose(stfp);

	// eigenval3 = allocvec(seq2len, sizeof(float), FALSE);
	// eigenvec3 = allocmat(seq2len, seq2len, sizeof(float), FALSE);
	 //eigen_decomposition(soltwomap, eigenval3, eigenvec3, seq2len);
	 //for (i=0; i < seq2len; i++)
	// eigenval3[i] = sqrtf(fabsf(eigenval3[i]));
	 //printf("eigenval3[i]===%f\n",eigenval3[i]);
	// freemat(soltwomap);
...
#else
	    strcpy(tdbname, "/share/home/zhanglab/xxy/Threader/Engerthread/database/AFDB_20W/tdb_eig_self_0.5/");
#endif
	...
	//比对得分的关键
	//hits[nn].path = path;
	// printf("hits[nn].path%s\n",hits[nn].path);
	seqfit(&hits[nn].contactsc, &hits[nn].perco_a, &hits[nn].perco_b, &tplt);

	if (hits[nn].contactsc >= SCUTOFF) //20
	{
		if (!(hits[nn].brkid = strdup(brkid)))
			fail("Out of memory!");

		//if (!(hits[nn].path = strdup(path)))
		//	fail("Out of memory!");

		if (mod3flg)
		{
		/**/
		//char  writepath1[] = "/share/home/zhanglab/xxy/meta_contact/test_291/test_fix1_30/";
		//strcat(writepath1, anaName);
		//strcat(writepath1, "_alignment.txt");
		//printf("============%s\n", writepath1);

		//FILE* fpWrite1 = fopen(writepath1, "a+");
		/*
		if (fpWrite1 != NULL)
		{
			printf("success!");
			//fprintf(fpWrite1, "111");
			//关闭文件

		}
		*/
		//fprintf(fpWrite1, "111%s\n", writepath1);

		//fprintf(fpWrite1, ">P1;%c%c%c%c\nstructureX:p%c%c%c%c:%s:%c:%s:%c:PDB::0.00:0.00\n", brkid[0], brkid[1], brkid[2], brkid[3], brkid[0], brkid[1], brkid[2], brkid[3], resid[0], brkid[4] == '0' ? ' ' : brkid[4], resid[seq1len - 1], brkid[4] == '0' ? ' ' : brkid[4]);
		...
		for (i = 1; i <= tplt.length; i++) 
			if (tplt.posn_a[i] <= 0){
				...
				//fprintf(fpWrite1, "%c", rescodes[seq1[tplt.posn_a[i] - 1]]);
			}
			else {
				putchar('X');
				//fprintf(fpWrite1, "X");
			...
			for (i = 1; i <= tplt.length; i++)
				if (tplt.posn_b[i] <= 0)
				{
					...
					//fprintf(fpWrite1, "%c", rescodes[seq2[tplt.posn_b[i] - 1]]);
				}
				else {
					putchar('X');
					//fprintf(fpWrite1, "X");
